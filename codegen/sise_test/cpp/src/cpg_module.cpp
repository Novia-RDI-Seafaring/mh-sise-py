...
CPG_Result_cpp_t solve_cpp(struct CPG_Updated_cpp_t& CPG_Updated_cpp, struct CPG_Params_cpp_t& CPG_Params_cpp){
    ...
    if (CPG_Updated_cpp.Q_w_sqrt) {
        for(i=0; i<9; i++) {
            cpg_update_Q_w_sqrt(i, CPG_Params_cpp.Q_w_sqrt[i]);
        }
    }
    if (CPG_Updated_cpp.Q_v_sqrt) {
        for(i=0; i<4; i++) {
            cpg_update_Q_v_sqrt(i, CPG_Params_cpp.Q_v_sqrt[i]);
        }
    }
    if (CPG_Updated_cpp.Q_u_sqrt) {
        for(i=0; i<4; i++) {
            cpg_update_Q_u_sqrt(i, CPG_Params_cpp.Q_u_sqrt[i]);
        }
    }
    ...
    for(i=0; i<200; i++) {
        CPG_Prim_cpp.vvar[i] = CPG_Prim.vvar[i];
    }
    for(i=0; i<198; i++) {
        CPG_Prim_cpp.uvar[i] = CPG_Prim.uvar[i];
    }
    for(i=0; i<300; i++) {
        CPG_Prim_cpp.xvar[i] = CPG_Prim.xvar[i];
    }
...
PYBIND11_MODULE(cpg_module, m) {
    ...
    py::class_<CPG_Params_cpp_t>(m, "cpg_params")
            .def(py::init<>())
            .def_readwrite("Q_w_sqrt", &CPG_Params_cpp_t::Q_w_sqrt)
            .def_readwrite("Q_v_sqrt", &CPG_Params_cpp_t::Q_v_sqrt)
            .def_readwrite("Q_u_sqrt", &CPG_Params_cpp_t::Q_u_sqrt)
    ...
    py::class_<CPG_Updated_cpp_t>(m, "cpg_updated")
            .def(py::init<>())
            .def_readwrite("Q_w_sqrt", &CPG_Updated_cpp_t::Q_w_sqrt)
            .def_readwrite("Q_v_sqrt", &CPG_Updated_cpp_t::Q_v_sqrt)
            .def_readwrite("Q_u_sqrt", &CPG_Updated_cpp_t::Q_u_sqrt)
    ...
    py::class_<CPG_Prim_cpp_t>(m, "cpg_prim")
            .def(py::init<>())
            .def_readwrite("vvar", &CPG_Prim_cpp_t::vvar)
            .def_readwrite("uvar", &CPG_Prim_cpp_t::uvar)
            .def_readwrite("xvar", &CPG_Prim_cpp_t::xvar)
            ;
